...
export default function CoursePreviewTemplate(props: 
    {
        data: any,
    ...
    if (!props.data) {
        return (
            <>
                ...
                }>Kurs suchen <ArrowUpRight /></Button>

                <ModalWrapper show={showSearch} onClose={() => {setShowSearch(false)}} title="Kurs suchen">
                    <CollectionQueryTableComponent collection={props.schema.preview} active={true} settings={{onSelectEntity: (entity: any) => {
                        ...
                        newObj.number_of_coaches = entity.data.coaches.length;
                        newObj.pool_id = entity.data.pool.id;
                        console.log(newObj);
                        setLocalData(newObj);
                        setHasChanged(true);
                        setShowSearch(false);
...

                {
                    localData ?
                        <CollapseWrapper dimension="height" show={!(props.depth <= 0 && !forceShowMore)}>
                            ...
                            <Row>
                                <Col sm={6}>
                                    <DataUI data={props.data.id} path={props.path ? props.path+"/id" : ""} depth={props.depth-1} onChange={(e:any) => {props.onChange(e)}} config={{...props.config, disableEditable: true}} />
                                </Col>
                                <Col sm={6}>
                                    <DataUI data={props.data.state} path={props.path ? props.path+"/state" : ""} depth={props.depth-1} onChange={(e:any) => {props.onChange(e)}} config={{...props.config, disableEditable: true}} />
                                </Col>
                            </Row>
